...
inline constexpr uint32_t kIrradianceFaceSize = 16;
inline constexpr uint32_t kSpecularBaseFaceSize = 64;
inline constexpr uint32_t kSpecularMipCount = 5;
inline constexpr uint32_t kDfgLutSize = 64;
...
inline constexpr uint32_t kSpecularSampleCount = 1024;
inline constexpr uint32_t kDfgSampleCount = 1024;
...
struct CpuCubemap {
    uint32_t faceSize = 0;
    std::array<std::vector<glm::vec4>, kCubeFaceCount> faces;
};
...
// mip 0 is a mirror resample of `env` and the last mip is roughness 1. Karis's split-sum prefilter:
// the normal, view and reflection vectors are all assumed equal to the texel direction, GGX
// importance sampling draws kSpecularSampleCount half-vectors from the Hammersley sequence, and the
// samples are averaged with N.L weights. The weights normalize, so a constant environment
...
struct IblTextures {
    std::unique_ptr<rhi::Texture> irradiance;
    std::unique_ptr<rhi::Texture> prefilteredEnv;
