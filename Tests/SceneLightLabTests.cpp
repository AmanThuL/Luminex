...
TEST_CASE("lightLabLights and lightLabTracks are identical across two builds",
    ...
    for (size_t i = 0; i < firstLights.size(); ++i) {
        ...
        const engine::LocalLight& b = secondLights[i];
        ...
        const auto firstRow = engine::makeLightRow(a);
        const auto secondRow = engine::makeLightRow(b);
        REQUIRE(firstRow);
        REQUIRE(secondRow);
        REQUIRE(std::memcmp(&*firstRow, &*secondRow, sizeof(engine::LightRow)) == 0);
    }
...
TEST_CASE("lightLabLights returns exactly n plus pile lights with one in four spot",
    ...
    for (const uint32_t n : {64u, 256u, 1000u, 4096u}) {
        ...
        for (const engine::LocalLight& light : lights) {
            spots += light.type == engine::LocalLightType::Spot;
        }
        REQUIRE(spots == n / 4);
    }
...
TEST_CASE("LightLab point and spot lights reach the floor at every benchmark population",
    ...
    for (const uint32_t n : {64u, 256u, 1024u, 4096u}) {
        ...
        for (const auto& light : lights) {
            ...
            REQUIRE(row);
            REQUIRE(engine::lightReaches(*row, floorPoint));
            const auto radiance = engine::computePunctualLight(
...
TEST_CASE("LightLab grid brightness stays comparable as ranges shrink", "[scene][light-lab]") {
    ...
    const auto response = [](const engine::LocalLight& light) {
        ...
        return engine::computePunctualLight(*row, floorPoint, {0, 1, 0}, {0, 1, 0}, glm::vec3(0.5f),
