...
TEST_CASE("external passes order inputs and outputs without opening an RHI scope",
    ...
    RenderGraph graph;
    const auto a = graph.importTexture(input, rhi::Format::RGBA16Float, "input");
    const auto b = graph.importTexture(output, rhi::Format::RGBA16Float, "output");
    const auto c = graph.importTexture(display, rhi::Format::BGRA8Unorm, "display");
    ...
    REQUIRE(record.debug.transitions.size() == 2);
    REQUIRE(record.debug.transitions[0].textureFrom == rhi::TextureUse::StorageWrite);
    REQUIRE(record.debug.transitions[0].textureTo == rhi::TextureUse::ExternalRead);
    REQUIRE(record.debug.transitions[1].textureFrom == rhi::TextureUse::ExternalWrite);
    REQUIRE(record.debug.transitions[1].textureTo == rhi::TextureUse::ShaderRead);
...
TEST_CASE("external terminal uses preserve cross-frame hazards", "[render][graph][external]") {
    ...
    SECTION("a prior external write is visible to a sampled read") {
        RenderGraph graph;
        const auto input = graph.importTexture(texture, rhi::Format::RGBA16Float, "history",
                                               rhi::TextureUse::ExternalWrite);
        const auto result = graph.importTexture(output, rhi::Format::RGBA16Float, "output");
        ...
        REQUIRE(record.has_value());
        REQUIRE(record->debug.transitions.size() == 1);
        REQUIRE(record->debug.transitions[0].textureFrom == rhi::TextureUse::ExternalWrite);
        REQUIRE(record->debug.transitions[0].textureTo == rhi::TextureUse::ShaderRead);
    }
    SECTION("a prior external read is ordered before a new write") {
        RenderGraph graph;
        const auto input = graph.importTexture(texture, rhi::Format::RGBA16Float, "history",
                                               rhi::TextureUse::ExternalRead);
        graph.addComputePass("overwrite", {.textureWrites = {input}}, kNoWork);
        graph.exportTexture(nextVersion(input));
        const auto record = graph.compileFrame(1);
        REQUIRE(record.has_value());
        REQUIRE(record->debug.transitions.size() == 1);
        REQUIRE(record->debug.transitions[0].textureFrom == rhi::TextureUse::ExternalRead);
        REQUIRE(record->debug.transitions[0].textureTo == rhi::TextureUse::StorageWrite);
    }
...
TEST_CASE("external declarations enforce texture versions and ranges",
    ...
    RenderGraph graph;
    const auto input = graph.importTexture(texture, rhi::Format::RGBA16Float, "mips");
...
TEST_CASE("external consumers bound transient lifetimes and unused work is culled",
    ...
    const auto result = graph.importTexture(output, rhi::Format::RGBA16Float, "result");
