...
// shouldResetExposure() (App/Model/ExposureReset.h, Tests/AppExposureResetTests.cpp) says a reset
// is pending: an ordinary computeExposureSeed dispatch that writes exp2(manualEV) into the
// persistent exposure buffer, no CPU readback involved, and no reason to run the same kernel four
// times over
