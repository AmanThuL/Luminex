...
First draft for owner revision. UX3 makes a scene a saved document instead of C++ code, rebuilds
...
explicit re-baseline. [Part IV](../../roadmap/editor-experience.md#ux3--scene-documents-and-hierarchy)
...
- The Hierarchy shows a `Workspace` root (Editor Camera, Rendering with eleven topics) beside the
  ...
  renderer configuration and scene content share one tree. [UX2](ux2.md) removes the Workspace
  root before UX3 starts.
...
| Hierarchy | Scene tree grouped by the document; look settings open in the Inspector from an Environment node. [UX2](ux2.md) already moves renderer configuration and live readings to a dockable Rendering panel and the editor camera out of the Hierarchy |
...
**Deliver:** the tree below with search, keyboard navigation and culled-row cues kept; the
Enabled checkbox in UX2's Inspector header extended from lights to objects; Exposure, Bloom and
Shadows leaving UX2's Rendering panel for the Environment node, since the document saves them;
File > Open, Save, Save As and Revert with native dialogs, a dirty marker, and a confirmation
before discarding edits. Transport Stop
...
relaunch and find all three; Revert restores the file's state; UX2's schema 4 workspaces open
without loss. Unverified gestures are recorded as unverified.
...
- **Spot direction round trip.** Direction to quaternion and back is not bit-exact, so UX3.1's
