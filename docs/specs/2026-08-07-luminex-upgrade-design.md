...
Luminex is the successor to [lumine](../../../lumine), a college-era DX12 renderer (shadow mapping w/ PCF, PCSS, sky, materials, ImGui inspector). The upgrade goals, in priority order:
...
- **Format**: `.clang-format` (C++23-aware; tuned in first PR), `xmake format` task, CI check (blocking).
- **Static analysis**: clang-tidy curated set, non-blocking CI job.
- **Tests** (`xmake test`): Catch2 unit tests for `Core` + CPU-side RHI logic; one **GPU smoke test** — offscreen triangle → readback → assert pixels. GPU test is a documented local gate; in CI it runs best-effort (macOS runners' virtualized GPU may lack Metal 4).
- **CI**: GitHub Actions `macos-26`: configure → build Debug+Release → unit tests → format check. Windows job returns with backend #2.
