...
The next acceptance gate is the implemented MetalFX temporal adapter, followed by the bounded display-path evaluation. GPU visibility and bounded local lighting follow the temporal slices. M5.6 closed as reliability failure / DEFER with no accepted performance conclusion (ADR 0012); it does not block M6 or preselect ICB. The [project-fit assessment](research/2026-09-06-graphics-paradigm-project-fit.md) also opens bounded neural-shader research without requiring completion of M8–M11.
