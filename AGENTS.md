...
pure editor/capture models, shared SceneSession and record observers; Tests compiles its own C++ only, mirroring `Source/` one level down under `Tests/` (a case sits beside the Source subject it asserts), with shared headers only in `Tests/Support/` and golden files only in `Tests/Golden/`; no SDL/ImGui/Metal/
