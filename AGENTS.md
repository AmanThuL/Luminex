...
- Roadmap entry: M6 has five temporal/display slices; M7 ends after five scene/visibility/lighting slices; M8 has five shadow/indirect/transparency/atmosphere slices; N1 has four inference-lab slices; `docs/roadmap.md#execution-sequence` owns the cross-part order: accepted post-M7 order R2 → R3 → R4 → UX2 → N1; R2.1 (RojoRHI conventions/ADRs in `rojo-rhi`, accepted ADR 0024) is owner-accepted 2026-09-19 with its plan closed, R2.2 is implemented and awaiting owner acceptance with its plan still open (`docs/plans/2026-09-19-r2.2-decouple-in-place.md`; `RHI/` builds/tests standalone, drops its Core dependency and links no Luminex target), the rest inactive, with records `docs/milestones/r2.md`, `r3.md`, `r4.md`, `ux2.md` (M8/M9 keep their identifiers though M9 delivers first); transparency belongs to M8, cluster LOD
