...
## Golden sources
- Founding design: `docs/decisions/0000-founding-design.md` (D1–D10 binding). Architecture: `docs/architecture/overview.md` indexes its seven subsystem pages (`core.md`, `engine.md`, `render-graph.md`, `render-passes.md`, `shaders.md`, `app.md`, `rojorhi.md`) and the frame walkthrough `docs/architecture/frame-pipeline.md`; guides: `docs/guides/gpu-debugging.md` · `docs/guides/temporal-comparison.md` · `docs/guides/screenshot-comparison.md`
...
pure editor/capture models, shared SceneSession and record observers; Tests compiles its own C++ only, with `Tests/<path>/` mirroring `Source/<path>/` (a case sits beside the Source subject it asserts), with shared headers only in `Tests/Support/` and golden files only in `Tests/Golden/`; no SDL/ImGui/Metal/
