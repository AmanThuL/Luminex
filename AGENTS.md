...
- Roadmap entry: M6 has five temporal/display slices; M7 ends after five scene/visibility/lighting slices; M8 has five shadow/indirect/transparency/atmosphere slices; N1 has four inference-lab slices; `docs/roadmap.md#execution-sequence` owns the cross-part order: accepted post-M7 order R2 → R3 → R4 → UX2 → N1, all inactive with `Proposed` records `docs/milestones/r2.md`, `r3.md`, `r4.md`, `ux2.md` (M8/M9 keep their identifiers though M9 delivers first); transparency belongs to M8, cluster LOD
